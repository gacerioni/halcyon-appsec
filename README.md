...
Product Security control repo for Halcyon Motors, a fictional EV maker with the same shape of problem as a
...
|---|---|---|---|---|
| halcyon-ota-campaign-service | Java 17 / Spring Boot 2.7 | yes | yes | snakeyaml 1.33 + `new Yaml()`, commons-text 1.9, old jackson |
| halcyon-warranty-claims-api | Java 17 / Spring Boot 2.7 | none | none | log4j-core 2.14.1 logging free text, snakeyaml 1.33, no tests |
| halcyon-owner-portal-bff | TypeScript / Fastify | yes | yes | jsonwebtoken 8.5.1 with no alg allowlist, axios 0.21.1, lodash 4.17.15 |
| halcyon-charging-network-gateway | TypeScript / Fastify | none | none | jsonwebtoken 8.5.1, node-fetch 2.6.0, minimist 1.2.5 |
| halcyon-telemetry-quality-checks | Python 3 / FastAPI | yes | yes | PyYAML 5.3 + `yaml.load` without Loader, `subprocess(shell=True)`, Jinja2 2.11 |
| halcyon-fleet-diagnostics-jobs | Python 3 | none | none | PyYAML 5.3, urllib3 1.26.4, cryptography 3.3.2, hardcoded service token |
| halcyon-vin-registry-service | Go 1.22 / SQLite | yes | yes | dgrijalva/jwt-go, x/crypto 0.14, SQL string concatenation |
| halcyon-service-appointments-api | Go 1.22 | partial | none | jwt-go without signing-method check, x/text 0.3.7 |
...
`rules/halcyon-appsec.yml` holds the custom Semgrep rules the security team wrote for the code-level
...
```bash
set -a; source ~/.halcyon-demo.env; set +a     # DEVIN_API_KEY, DEVIN_ORG_ID (never committed)
